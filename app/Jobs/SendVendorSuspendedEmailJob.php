...
class SendVendorSuspendedEmailJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if (! $vendor) {
            Log::warning('SendVendorSuspendedEmailJob: vendor not found', ['vendor_id' => $this->vendorId]);
            return;
        }
        ...
        if (\Illuminate\Support\Facades\Cache::has($sentKey)) {
            Log::debug('SendVendorSuspendedEmailJob: already sent (idempotent skip)', ['vendor_id' => $vendor->id]);
            return;
        }

        $processingKey = $sentKey . '_processing';
        if (! \Illuminate\Support\Facades\Cache::add($processingKey, true, 60)) {
            Log::debug('SendVendorSuspendedEmailJob: already processing (concurrent skip)', ['vendor_id' => $vendor->id]);
            return;
        }

        $email = $vendor->user?->email;
        if (! $email) {
            Log::warning('SendVendorSuspendedEmailJob: vendor user email missing', ['vendor_id' => $vendor->id]);
            return;
        }

        try {
            $mailable = (new VendorSuspendedMail([
                'vendor' => $vendor,
                'user' => $vendor->user,
            ]))->onQueue(config('queues.inventory'));
            Mail::to($email)->queue($mailable);

            Log::info('Email queued: VendorSuspendedMail', [
                'vendor_id' => $vendor->id,
                'to' => $email,
            ]);
