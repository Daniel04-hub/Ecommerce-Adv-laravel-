...
class SendProductRejectedEmailJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if (! $product) {
            Log::warning('SendProductRejectedEmailJob: product not found', ['product_id' => $this->productId]);
            return;
        }
        ...
        if (\Illuminate\Support\Facades\Cache::has($sentKey)) {
            Log::debug('SendProductRejectedEmailJob: already sent (idempotent skip)', ['product_id' => $product->id]);
            return;
        }

        $processingKey = $sentKey . '_processing';
        if (! \Illuminate\Support\Facades\Cache::add($processingKey, true, 60)) {
            Log::debug('SendProductRejectedEmailJob: already processing (concurrent skip)', ['product_id' => $product->id]);
            return;
        }

        $email = $product->vendor?->user?->email;
        if (! $email) {
            Log::warning('SendProductRejectedEmailJob: vendor user email missing', ['product_id' => $product->id]);
            return;
        }

        try {
            $mailable = (new ProductRejectedMail([
                'product' => $product,
            ...
            Log::info('Email queued: ProductRejectedMail', [
